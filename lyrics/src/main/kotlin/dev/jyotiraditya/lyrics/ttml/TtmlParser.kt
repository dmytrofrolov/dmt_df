 ...
 * - `ttm:role="x-bg"` on a span marks backing/adlib vocals. Such a span often runs past the `end`
 *   its `<p>` carries, since that end is when the lead vocal stops rather than when the line is
 *   done, so a line runs until the last of its words rather than until its own `end`.
                        ...
                        "p" -> if (inLine) {
                            ...
                            if (lineText.isNotEmpty()) {
                                ...
                                lines += LyricLine(
                                    startMs = minOf(
                                        lineBegin,
                                        bounded.minOfOrNull { it.startMs } ?: lineBegin,
                                    ),
                                    endMs = maxOf(
                                        lineEnd,
                                        bounded.maxOfOrNull { it.endMs } ?: lineEnd,
                                    ),
                                    text = lineText,
                                    words = bounded,
                                    voice = lineVoice,
